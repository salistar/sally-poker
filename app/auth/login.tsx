...
export default function LoginScreen() {
  ...
  const { promptAsync, ready: googleReady, missingClientId, missingNative } = useGoogleSignIn(
    async (idToken: string) => {
      setLoading(true);
      try {
        await api.loginWithGoogle(idToken, { gameType: 'poker' });
        console.log('[Poker/Login] Google login successful');
        router.replace('/(tabs)');
      } catch (e: any) {
        console.error('[Poker/Login] Google error:', e);
        Alert.alert(t('error'), e.message || 'Google sign-in failed');
      } finally {
        setLoading(false);
      }
    },
    (msg: string) => { console.warn('[Poker/Login] Google:', msg); setLoading(false); }
  );
