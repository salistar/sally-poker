 ...
 * @description Leaderboard Poker avec 3 portées : Mondial / Pays / Ville.
 * Données réelles depuis /leaderboards/poker?scope=<...>, auto-détection
...
export default function LeaderboardScreen() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      log.bin(`GET /leaderboards/poker`, { scope, filter });
      const r = await api.getLeaderboardScoped('poker', filter, scope, 50);
      log.bout('200 /leaderboards/poker', `${r.entries.length} entrées`);
