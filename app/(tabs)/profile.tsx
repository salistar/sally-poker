 ...
 * @description Profil Poker avec vraies valeurs (stats/coins/achievements/
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 2) /leaderboards/poker/my-rank : best-effort
      try {
        log.bin('GET /leaderboards/poker/my-rank');
        const r = await api.getMyRank('poker', 'season');
        ...
        const msg = e?.message ?? e?.toString?.() ?? 'unknown';
        const status = e?.status ?? '?';
        if (status === 404) log.explain('pas encore de rang poker pour ce joueur');
