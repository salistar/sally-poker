/**
 * @file (tabs)/index.tsx
 * @description Home Poker. 4 actions principales : Créer, Rejoindre, vs Bot
...
export default function HomeScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setUser(u);
      log.bin('GET /challenges/daily/poker');
      const ch = await api.getDailyChallenge('poker');
      if (ch) {
        log.bout('200 /challenges/daily/poker', { title: ch.title, reward: ch.rewardCoins });
        setChallenge(ch);
      }
  ...
  const handleChallenge = async () => {
    log.bin('POST /challenges/daily/poker/matchmake');
    try {
      const room = await api.joinDailyChallenge('poker');
